...
function AgentAvatar({
  ...
  const bgClass =
    role === 'user'
      ...
      : 'bg-white/[0.05] border-white/[0.07] text-muted-foreground';
...
function RoleBadge({ role, isToolCall }: { role: MessageRole; isToolCall?: boolean }) {
  ...
  const map: Record<MessageRole, string> = {
    ...
    assistant: 'bg-white/[0.05] text-muted-foreground border-white/[0.07]',
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-2 flex-wrap mb-1">
          {msg.agentName && (
            <span className="text-xs font-medium text-white/90">{msg.agentName}</span>
          )}
        ...
        {isToolCall ? (
          <div className="rounded-card bg-secondary/5 border border-secondary/10 px-3 py-2">
            {msg.content && (
              <p className="text-xs text-white/70 mb-1.5 leading-relaxed">{msg.content}</p>
            )}
...
export function ConversationFeed() {
  ...
  const handleClearFeed = () => {
    setLocalMessages([]);
    // Note: clearing live events requires clearing from the store;
    // here we just signal it visually by clearing local additions.
    // A full implementation would call something like clearRecentEvents() from the store.
  };
    ...
    <GlassPanel className="flex flex-col overflow-hidden" style={{ minHeight: 400 }}>
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-white/[0.07] flex-shrink-0">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-sm font-semibold text-white">Conversation Feed</h2>
        ...
        <button
          ...
          className="text-xs text-muted-foreground hover:text-white transition-colors flex items-center gap-1.5 px-2 py-1 rounded-card hover:bg-white/[0.05]"
        >
